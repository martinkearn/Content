...
I was recently involved in a short, 4-day customer hack based on Microsoft Bot Framework V4 (C# SDK), Azure Bot Service and Language Understanding Intelligence Service (part of Cognitive Services).

I learnt a lot in the 4 days and have some observations which I figured might be helpful to anyone else who is thinking about Bot Framework V4 (I’ll refer to it as 'BFv4' from now on), what state it is in right now and whether it is worth seriously looking at yet. 
...
BFv4 is currently in public preview which was announced at the Build conference back in May 2018. We do not have any dates for the final release. [Read the initial announcement here](https://blog.botframework.com/2018/05/07/build-a-microsoft-bot-framework-bot-with-the-bot-builder-sdk-v4/)

It is worth noting that I'm writing from a perspective of someone who has done a fair few bots using the V3 framework using almost exclusively C#. I’m also reasonably well experienced with both Azure Bot Service and LUIS for use with V3 bots. As you’ll see when you read on, V3 experience is of limited value for BFv4, in some ways it could even make things harder to figure out.
...
* **More Languages.** BFv4 SDK is available for NodeJS / JavaScript, C#, Python and Java.
...
* **Overall service architecture remains the same**. The concept of the bot as a single code based which is published to multiple channels (Skype, Cortana, Facebook etc) remains the same. The Azure Bot Service idea also remains the same.
* **.net Core 2**. For the .netters amongst you, you’ll be glad to learn that the .net BFv4 SDK is built on .net core 2.0 which means that you get to use all those cool .net core features like middleware and DI. [See the open source .net core SDK here](https://github.com/Microsoft/botbuilder-dotnet)
...
* **Dialogs and Conversation flow**: The way conversation flow has changed fairly significantly compared to BFv3. key docs include [Manage conversation flow with dialogs](https://docs.microsoft.com/en-us/azure/bot-service/bot-builder-dialog-manage-conversation-flow?view=azure-bot-service-4.0&tabs=csharp) and [Create modular bot logic with a dialog container](https://docs.microsoft.com/en-us/azure/bot-service/bot-builder-compositcontrol?view=azure-bot-service-4.0&tabs=csharp). These are some of the key concepts:
    ...
    * **Prompt**: A `Prompt` is a type of built in dialog intended to capture specific pre-defined data from the user such as text, numbers, dates, confirmation or choices. Conceptually this is the same as BFv3. Prompts have built in capabilities for handling verification
...
### Docs
The docs are actually in quite decent shape and the main, high level conceptual stuff is documented fairly well. 
...
If you try to search for them, you may end of on the V3 docs, but if you hit the 'Azure Bot Service' drop-down in the top left corner and select "SDK V4.x (preview)", it will filter on the BFv4 docs.

### Samples
As is usual, the samples are typically update don GitHub first and then updated into the docs at a later stage, so I'd always use GitHub as you go to location for code samples and examples.
...
The best samples I found for .net were on a specific `contosocafe-v4-dotnet` branch of the general [BotFramework-Samples](http://github.com/Microsoft/BotFramework-Samples) repository. The samples are focused on a Contoso Cafe scenario and seem to be very fresh (last updated 29th June 2018): https://github.com/Microsoft/BotFramework-Samples/tree/contosocafe-v4-dotnet/docs-samples/V4/dotnet/ContosoCafe/ContosoCafe-5-DialogsWithLUISEntities/ContosoCafe

### Azure Bot Service
A bot is essentially a web API and so in theory, it can be hosted on any web service. However, it seems very clear to me that the intention is that bots are hosted as part of the [Azure Bot Service](https://docs.microsoft.com/en-gb/azure/bot-service/?view=azure-bot-service-4.0).
...
That said, the option is yours but my advice would be that if you are going to 'stray from the beaten path' and host elsewhere, you may find that many of the docs and samples become harder to follow.

### Luis Middleware & Strongly Typed Class
As with BFv3, almost every bot will need some level of natural language processing which is where [Cognitive Services Language Understanding Service (LUIS)](https://azure.microsoft.com/en-us/services/cognitive-services/language-understanding-intelligent-service/) comes in. Luis extract intent and entities from a user's utterance.

Intent is normally used to define the top level branch of the conversation.
...
However, there are a few issues with the `LuisRecognizerMiddleware` as it stands:
1. Luis is generally used at the top of the conversation; once you have the intent and entities, Luis is no longer required. However, middleware is executed on every Turn, which will become expensive and wastefull in terms of bandwith for most scenarios.
2. The `LuisRecognizerMiddleware` does not provide a strongly typed object to work with. You can still get to the same data but it is very complex to work with.

An alternative approach is to use a `LuisRecognizer` as required in you bot's root dialog set to gather top level intent and entities and then brand out to DialogConatiners from there, thus reducing the use of Luis.
...
The [Extract intents and entities using LUISGen](https://docs.microsoft.com/en-us/azure/bot-service/bot-builder-howto-v4-luisgen?view=azure-bot-service-4.0&tabs=cs) docs give you a high level overview of this approach, however at teh time of writing the sample were incomplete and difficult to follow.

For a complete example of the `LuisRecognizer` in action, please see my [Bot-V4-Banko ](https://github.com/martinkearn/Bot-V4-Banko) example on GitHub. This is based on a fictitious bank which enable balance checks and transfer via their bot. This is also a good example of multiple DialogContainers in use.
...
However, as with all platform re-writes of this nature, BFv4 is really a very good developer platform and once the initial learning curve has been overcome, i think it is a much easier and overall better platform for writing bots compared to BFv3.
